...
public final class PageComponentEffect
  ...
  private void init() {
    if (this.init) return;
    this.texture = Herbarium.proxy.getClient()
                                  .getTextureManager()
                                  .getDynamicTextureLocation(this.uuid, new DynamicTexture(this.image));
    this.init = true;
  }
  ...
  public void render(int x, int y, float partial) {
    ...
    String[] lines = this.wrap(this.text, 17)
                         .split("\n");
    ...
    dY += pad;
    dX = pad;
    lines = this.wrap(this.text.substring(startX), 25)
                .split("\n");
    for (String line : lines) {
      GlStateManager.pushMatrix();
