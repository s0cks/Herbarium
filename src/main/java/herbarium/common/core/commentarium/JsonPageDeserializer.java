...
public final class JsonPageDeserializer
  ...
  throws JsonParseException {
    ...
    String title = ctx.get("title")
                      .getAsString();
    IPageGroup group = PageGroups.valueOf(ctx.get("group")
                                             .getAsString()
                                             .toUpperCase());
    IPageLayout layout;
    ...
    if (layoutElem.isJsonObject()) {
      ...
      switch (layoutObj.get("name")
                       .getAsString()
                       .toLowerCase()) {
        case "grid": {
          int rows = layoutObj.get("rows")
                              .getAsInt();
          int columns = layoutObj.get("columns")
                                 .getAsInt();
          layout = new PageLayoutGrid(rows, columns);
          break;
        }
        default: {
          throw new JsonParseException("Unknown layout type: " + layoutObj.get("name")
                                                                          .getAsString()
                                                                          .toLowerCase());
        }
      }
    ...
    JsonArray components = ctx.get("components")
                              .getAsJsonArray();
    for (int i = 0; i < components.size(); i++) {
      JsonObject componentObj = components.get(i)
                                          .getAsJsonObject();
      IPageComponent comp;
      switch (componentObj.get("type")
                          .getAsString()
                          .toLowerCase()) {
        case "image":
          comp = new PageComponentImage(componentObj.get("ref")
                                                    .getAsString());
          break;
        case "text":
          comp = new PageComponentText(componentObj.get("text")
                                                   .getAsString());
          break;
        case "effect": {
          IEffect effect = HerbariumApi.EFFECT_MANAGER.getEffect(componentObj.get("effect")
                                                                             .getAsString());
          comp = new PageComponentEffect(effect, componentObj.get("text")
                                                             .getAsString());
          break;
        }
        default: {
          throw new JsonParseException("Unknown component type: " + componentObj.get("type")
                                                                                .getAsString());
        }
      }
      if (layout instanceof PageLayoutBorder) {
        ((PageLayoutBorder) layout).add(comp, PageLayoutBorder.Position.valueOf(componentObj.get("position")
                                                                                            .getAsString()
                                                                                            .toUpperCase()));
      }
      renderer.add(comp);
