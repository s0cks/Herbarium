...
public final class Ruin{
  ...
  public IBlockState getBlockState(BlockPos pos){
    ...
    return Block.getBlockById(this.blocks[x][y][z]).getStateFromMeta(this.metadata[x][y][z]);
  }
  ...
  public static Ruin create(ResourceLocation loc){
    try(InputStream in = Herbarium.proxy.getClient().getResourceManager().getResource(loc).getInputStream()){
    ...
    for(int x = 0; x < width; x++){
      for(int y = 0; y < height; y++){
        for(int z = 0; z < length; z++){
          ...
          try{
            if(!ruin.setBlockState(new BlockPos(x, y, z), Block.getBlockById(blockID).getStateFromMeta(meta))) System.out.println("Cant add block");
          } catch(Exception e){
